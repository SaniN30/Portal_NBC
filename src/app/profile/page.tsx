...
type Me = {
  ...
  email: string | null; role: string; resumeBlobUrl: string | null; resumeUploadedAt: string | null;
};
  ...
  async function save(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const updated = await api<Me>("/api/me", {
        method: "PUT",
        body: JSON.stringify({ fullName: f.get("fullName"), dob: f.get("dob"), phone: f.get("phone"), email: f.get("email") }),
      });
      setMe((m) => ({ ...m!, ...updated }));
      setMsg("Saved.");
